...
class Edit_Expense_Transaction(Resource):
    ...
    def put(self, expense_transaction_id):
        user_id = check_token(request)
        expense_transaction_id = validate_expense_transaction_id(
        ...
        edit_expense_transaction(
            expense_transaction_id, amount, category, description, date)
        return
...
class Get_Expense_Transaction(Resource):
    ...
    # Assuming you have a transaction_model defined
    @api.response(200, "Success")
    ...
    def get(self):
        ...
        serialized_transactions = [serialize_expense_transaction(
